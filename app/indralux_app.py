...
def get_channel_map():
    st.sidebar.markdown("### Channel Mapping")
    r = st.sidebar.selectbox("F-actin (Cytoskeleton)", ["Red", "Green", "Blue"])
    g = st.sidebar.selectbox("VE-cadherin (Junction)", ["Red", "Green", "Blue"])
    b = st.sidebar.selectbox("DAPI (Nuclei)", ["Red", "Green", "Blue"])
    idx = {"Red": 0, "Green": 1, "Blue": 2}
    return {"F-actin": idx[r], "VE-cadherin": idx[g], "DAPI": idx[b]}
            ...
            if st.session_state[run_key]:
                ...
                for idx, col_path in enumerate(col_paths):
                    try:
                        ...
                        df, labels, img_rgb = process_with_breaks(col_path, n_columns=1, column_labels=[label], channel_map=channel_map)
                        ...
                        df = add_ve_snr(df, labels, img_rgb[:, channel_map.get("VE-cadherin", 1)])
...
# Single Image Mode
    ...
    if uploaded_file:
        ...
        with st.spinner("Processing image..."):
            try:
                df, labels, img_rgb = process_with_breaks(img_path, n_columns=len(column_labels), column_labels=column_labels, channel_map=channel_map)
